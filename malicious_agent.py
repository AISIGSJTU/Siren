...
def benign_train(x, y, agent_model, logits, X_shard, Y_shard, sess, shared_weights):
    ...
    offset_temp = 0
    loss_val_shard = 0.0
    for step_temp in range(int(num_steps_temp)):
...
def alternate_train(sess, t, optimizer, loss, mal_optimizer, mal_loss, x, y,
    ...
    while step < num_steps:
        ...
        if step < num_steps:
            ...
            # print("mal_loss_val_bef:", mal_loss_val_bef[0])
...
def mal_agent_other(i, X_shard, Y_shard, t, gpu_id, return_dict, X_test, Y_test, lr=None):
    ...
    if args.attack_type == 'label_flipping':
        ...
        for w in range(Y_shard.shape[0]):
            if Y_shard[w][0] == 1.0:
                # print("original",i, Y_shard[i])
                Y_shard[w].itemset(0, 0.0)
                Y_shard[w].itemset(9, 1.0)
                # print("now",i,Y_shard[i])
            elif Y_shard[w][1] == 1.0:
                Y_shard[w].itemset(1, 0.0)
                Y_shard[w].itemset(8, 1.0)
            elif Y_shard[w][2] == 1.0:
                Y_shard[w].itemset(2, 0.0)
                Y_shard[w].itemset(7, 1.0)
            elif Y_shard[w][3] == 1.0:
                Y_shard[w].itemset(3, 0.0)
                Y_shard[w].itemset(6, 1.0)
            elif Y_shard[w][4] == 1.0:
                Y_shard[w].itemset(4, 0.0)
                Y_shard[w].itemset(5, 1.0)
            elif Y_shard[w][5] == 1.0:
                Y_shard[w].itemset(5, 0.0)
                Y_shard[w].itemset(4, 1.0)
            elif Y_shard[w][6] == 1.0:
                Y_shard[w].itemset(6, 0.0)
                Y_shard[w].itemset(3, 1.0)
            elif Y_shard[w][7] == 1.0:
                Y_shard[w].itemset(7, 0.0)
                Y_shard[w].itemset(2, 1.0)
            elif Y_shard[w][8] == 1.0:
                Y_shard[w].itemset(8, 0.0)
                Y_shard[w].itemset(1, 1.0)
            elif Y_shard[w][9] == 1.0:
                Y_shard[w].itemset(9, 0.0)
                Y_shard[w].itemset(0, 1.0)
    ...
    for step in range(num_steps):
        ...
        if step % 1000 == 0:
            print('Mal_Agent %s, Step %s, Loss %s, offset %s' % (i, step, loss_val, offset))
    ...
    print('Mal_Agent: success {}, loss {}'.format(eval_success, eval_loss))
    with open('output/client_%s.txt' % i, 'a') as f:
        f.write('Mal_Agent {}: success {}, loss {}\n'.format(i, eval_success, eval_loss))

    return_dict[str(i)] = np.array(local_delta)

    np.save(gv.dir_name + 'mal_delta_%s_t%s.npy' % (i, t), local_delta)
    if t > 0 and os.path.exists(gv.dir_name + 'mal_delta_%s_t%s.npy' % (i,t-1)):
        os.remove(gv.dir_name + 'mal_delta_%s_t%s.npy' % (i,t-1))
